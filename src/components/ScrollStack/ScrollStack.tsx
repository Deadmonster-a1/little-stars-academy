...
const ScrollStack = ({
  ...
  const getElementOffset = useCallback(
    (element: HTMLElement) => {
      if (useWindowScroll) {
        // Get the static offset of the scroller relative to the document
        const scroller = scrollerRef.current;
        const scrollerTop = scroller ? scroller.getBoundingClientRect().top + window.scrollY : 0;
        // Use offsetTop instead of getBoundingClientRect().top for the element
        // because the element is actively being transformed (translated), which 
        // affects getBoundingClientRect() and causes a jitter/feedback loop.
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Skip instantiating a new Lenis on the window, since the app already has a global Lenis instance.
      // Hooking into RAF will perfectly sync with the global Lenis scroll updates.
